...
- 전체 빌드는 197초 — 커서 6,099개(57종 × 107칸 = 기본 17 + 매끈한 모양 10가지 × 9, 각각 3크기),
  코어 수만큼 프로세스, 2026-09-20 점검 (`smooth.FPS = 30` 을 켜기 전에는 147–161초였다. 움직이는 칸의
  프레임이 2~4배로 늘어 그린 그림을 PNG 로 굽는 값이 그만큼 늘었다. 121종이던 때는 224–268초였고
  그중 146초가 순수 규모였다). 산출물은 `dist` 247MB + `data` 47MB — **`FPS` 를 끄면 dist 가 121MB 로
  ...
  고친 것만 다시 그린다 (아무것도 안 고쳤으면 1.5초 — `preview.html` 은 늘 다시 만든다, 구성표 하나면 12초). 전부 다시 그리려면 `python build.py --all`
- CI 도 `dist`·`data`·`.build-stamp.json`·`.stencils.pkl` 을 캐시해 같은 수를 쓴다 — 히트면 잡 51초(빌드 8초),
  미스면 10분(빌드 스텝 9분) — 2026-09-20 실측. 121종 때는 12분이었다: 커서가 절반이 됐는데 15% 만 준 것이고 왜인지는 안 쟀다. **캐시는 브랜치마다 따로 저장되고**, 다른 브랜치가 받아 쓸 수 있는 것은
