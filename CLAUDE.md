...
- 전체 빌드(`--all`)는 **사용자 PC(Ryzen 5 5600X, 워커 12개) 262.4초 @ d33cecab, 2026-09-24** — 커서 6,099개
  ...
  **그림에 프레임을 더 그리면 빌드가 그만큼 길어진다** — 전기 32장·용암 24장·무지개 흐름 24장으로 늘린 뒤
  ...
  구성표가 한 덩이에 몰려 끝에 워커들이 놀았다(PC 262초 중 22초 어림). **시안 데이터(`data/<모양>.json`)도 같은 일감이
  ...
  2026-09-20 PC 147–161초였고, 121종이던 때는 224–268초였다). 산출물은 `dist` 247MB + `data` 47MB — **`FPS` 를 끄면 dist 가 121MB 로
