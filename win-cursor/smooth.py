...
SWAY = 0.96     # 프레임 전부에 공통인 몸이 장 몸 평균의 이 비율 아래면 **몸이 흔들리는 그림**(헤엄)으로 보고
                # 번짐을 그 장의 몸 밖에서만 뜬다 (samplers_of). schemes.json 에 sway 를 단 구성표에서만 —
                # 해양 애니는 0.74–0.95 인데 반짝이 move 0.92·폭죽 move 0.94·지터 ibeam 0.92 도 이 아래라
                # 비율로는 못 가른다 (2026-09-24 잰 값)
...
def samplers_of(frames: list[dict], mat: str | None = None, sway: bool = False) -> list[tuple]:
    ...
    sway = sway and len(core) * len(bodies) < SWAY * sum(map(len, bodies))
...
def drawer(sid: str, rid: str, frames: list[dict], glyphs: list | None = None, mat: str | None = None,
    ...
    samplers = samplers_of(frames, mat, sway)
