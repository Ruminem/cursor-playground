...
리눅스 컨테이너에서 조사했다. 맥에서 직접 본 것은 ① 시험판 하나뿐이다 — "맥에서 재 본 결과"와 맨 아래 "맥에서 확인할 것" 참고.
...
### 맥에서 재 본 결과 (2026-09-24, 사용자 맥)
- **된다.** Mousecape-swiftUI 로 `electric.cape` 를 가져와 적용하니 시스템 커서가 전기로 바뀌었다
- Gatekeeper 가 처음 열 때 막았다("악성 코드가 없음을 확인할 수 없습니다") — 시스템 설정 › 개인정보 보호 및 보안 › "그래도 열기" 로 넘었다.
  사람들에게 권하려면 이 한 단계를 안내에 꼭 적어야 한다
- 화살표가 번쩍이며 돌고, I빔·손가락도 바뀌고, 한 바퀴가 1초쯤으로 돌고, 다른 앱으로 넘어가도 남았다 (사용자 눈 확인)
- 재부팅해도 남았다
- **안 본 것**: 아래 목록 4~8

### 다음 시험: 폭죽 (가장 많이 솎이는 것)
전기가 되어서 57종으로 넓히기 전에, 가장 많이 잃는 폭죽을 본다. `python gen/cape.py firework` → 285 KiB,
원본 144장 × 1틱(2.4초, 60fps) → 24장 × 0.1초(10fps). 여섯 장에 한 장만 남아서 1~2장짜리 번쩍임·흰 십자는 빠질 수 있다.
맥에서 봐줄 만하면 ②-B(시안 페이지에서 57종 굽기)로 가고, 못 봐주겠으면 여기서 접는다.

### 폭죽 결과 (2026-09-24)
사용자 맥에서 "봐줄 만함". 그래서 ②-B 로 갔다.

## ② 시안 페이지 버튼: 맥용 .cape 받기 (2026-09-24)

윈도우가 아니면 화면 맨 아래 줄의 **적용** 자리가 **맥용 .cape 받기** 로 바뀌어, **지금 고른 구성표·모양·색조**를 `.cape` 로 구워 내려준다.
윈도우는 바로 적용, 맥은 받기지만 목적이 같아서 자리도 같다 (처음엔 "맥에서 쓰기" 칸에 따로 뒀다가 09-24 옮김). 맥에선 그 줄의 원래대로는 숨긴다 — Mousecape 의 ⌘R 이 그 일.

- 새로 그리지 않는다. 윈도우 적용과 같은 `dist/[<모양>/]<구성표>/<칸>.cur|ani` 를 받아(모양 쪽에 없으면 기본 것 — `handler.ps1` 과 같은 규칙)
  `.cur` 안의 32·64px PNG 를 꺼내 캔버스로 세로로 쌓는다. 그래서 빌드·dist 용량은 그대로다
- 장 고르기: 한 바퀴를 k 등분해 그때 보이는 장을 고른다. 고르게 도는 n 장이면 k = min(n, 24) 라 `gen/cape.py` 와 같은 장이고,
  매끈한 모양처럼 틱이 고르지 않으면(3+2 …) k = min(전체 틱, 24). 어느 쪽이든 한 바퀴 시간은 원래와 같다
- 색조는 페이지 미리보기와 같은 `rotate`(= `handler.ps1` 의 Recolor 식)로 돌린다
- 칸 ↔ 맥 이름은 `win-cursor/mac.json` 하나. `gen/cape.py` 도 여기서 읽는다
- 컨테이너 확인: dist 몇 벌을 `build.cursor_bytes` 로 만들어 Playwright 로 눌렀다. 기본 모양 전기·분홍은 `gen/cape.py` 와
  픽셀·핫스팟·장 수·길이가 같고, 폭죽은 반투명 칸 몇 개가 캔버스 알파 반올림으로 최대 2/255 다르다. 둥근 전기 24장 × 0.0444초(원래 1.07초),
  분홍 둥근 120° 는 초록으로 돈다
- 캔버스 PNG 라 `gen/cape.py` 보다 3배쯤 크다 (전기 709KiB, 폭죽 855KiB). 문제되면 줄일 곳이 여기다
- 파일 이름 `cursor-playground-<구성표>[-<모양>][-h<색조>].cape`, Identifier 도 같은 꼴이라 여러 벌을 Mousecape 에 같이 넣을 수 있다

## 맥에서 확인할 것

1. ~~Mousecape-swiftUI 가 설치되는지, Gatekeeper 가 막는지~~ — 막지만 "그래도 열기"로 된다 (09-24)
2. ~~우리 `.cape` 를 읽고 적용하는지~~ 된다 (09-24). 재부팅 뒤에도 남는다 (09-24)
3. 화살표가 메뉴 막대·Dock·Finder·브라우저 어디서나 바뀌는지 (26 의 화살표 이름 문제) — 앱을 넘나들어도 남는 것은 봤다
